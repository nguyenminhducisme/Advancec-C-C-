...
/* Khai báo các kiểu dữ liệu sẽ xuất hiện trong chuỗi Json*/
typedef enum {
    JSON_NULL,
    JSON_BOOLEAN,
    JSON_NUMBER,
...
typedef struct JsonValue {
    JsonType type;
    union { // Lý do dùng union ở đây vì ở 1 thời điểm ta chỉ thao tác với 1 dữ liệu nên dùng union cho tiết kiệm dữ liệu
        int boolean;
        double number;
        char *string;
        struct {
...
JsonValue *parse_json(const char **json); // 1 con trỏ hàm để thao tác

void free_json_value(JsonValue *json_value);

/* Hàm bỏ qua khoảng trắng của chuỗi, ở đây ta khai báo static bởi vì để tránh việc user lấy ra sử dụng*/
